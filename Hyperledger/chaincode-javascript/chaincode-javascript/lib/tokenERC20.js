...
class TokenERC20Contract extends Contract {
    ...
    //刪除某Address
    async delUserData(ctx, userAddress) {
        ...
        if (!userCurrentBalanceBytes || userCurrentBalanceBytes.length === 0) {
            throw new Error(`client account ${userAddress} 不能刪除`);
        }

        await ctx.stub.deleteState(userBalanceKey);
        return true;
    ...
    */
    ...
    async _transfer(ctx, from, to, value) {
        ...
        await this.setAdminTransactionData(ctx, from, to, value);
    ...
    async TransferFrom(ctx, from, to, value) {
        ...
        if (!fromCurrentBalanceBytes || fromCurrentBalanceBytes.length === 0) {
            throw new Error(`client account ${from} 沒有餘額`);
        }
        ...
        // Check if the sender has enough tokens to spend.
        if (fromCurrentBalance < valueInt) {
            throw new Error(`client account ${from} 資金不足`);
        }
    ...
    async setFromTransactionData(ctx, from, to, value) {
        ...
        if (!transactionDataBytes || transactionDataBytes.length === 0) {
            const transactionData = [{
                ...
                to: to,
                value: value,
                time: ctx.stub.getTxTimestamp().seconds.toString()
            }]
    ...
    // 刪除某Address的所有交易資訊
    // async delTransactionData(ctx, userAddress) {
    ...
    //     if (!transactionDataBytes || transactionDataBytes.length === 0) {
    //         throw new Error(`client account ${userAddress} 沒有交易紀錄不能刪除`);
    //     }

    //     await ctx.stub.deleteState(transactionDataKey);
    //     return true;
